...
resource "aws_cloudwatch_event_target" "lambda" {
  ...
  count     = "${local.schedule_enabled && ! var.attach_vpc_config && ! var.attach_dead_letter_config ? 1 : 0}"
}
...
resource "aws_cloudwatch_event_target" "lambda_with_dl" {
  ...
  count     = "${local.schedule_enabled && var.attach_dead_letter_config && ! var.attach_vpc_config ? 1 : 0}"
}
